...
public class MainActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		recordBtn.setOnClickListener(v -> {
			if (hasAllPermissions())				openSaveLocation();
			else Toast.makeText(this, "Missing permissions!", Toast.LENGTH_SHORT).show();
		});
		stopBtn.setOnClickListener(v -> stopRecording());
		playBtn.setOnClickListener(v -> {
			if (videoUri != null) {
				videoView.setVideoURI(videoUri);	videoView.start();
			} else Toast.makeText(this, "No video to play", Toast.LENGTH_SHORT).show();
		});
	}
	private void openSaveLocation() {
		...
		if (uri != null) {videoUri = uri;
			if (surfaceReady) setupMediaRecorder();
			else Toast.makeText(this, "Surface not ready yet.", Toast.LENGTH_SHORT).show();
		} else Toast.makeText(this, "Failed to create video Uri", Toast.LENGTH_SHORT).show();
	}

	private void setupMediaRecorder() {
		try {FileDescriptor fd = getContentResolver().openFileDescriptor(videoUri, "w").getFileDescriptor();
	...
	public void onRequestPermissionsResult(int requestCode,
		...
		if (requestCode == REQUEST_PERMISSIONS) { if (hasAllPermissions())
